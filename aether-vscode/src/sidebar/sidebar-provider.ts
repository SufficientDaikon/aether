...
export class AetherSidebarProvider implements vscode.WebviewViewProvider {
  public static readonly viewId = "aether.sidebar";
  ...
  public navigateTo(panel: string): void {
    this._view?.webview.postMessage({ type: "navigate", panel });
  }

  public postEvent(event: string, data: any): void {
    this._view?.webview.postMessage({ type: "event", event, data });
  }
  ...
  private async handleMessage(message: any): Promise<void> {
    ...
    try {
      const result = await this.executeRPC(message.method, message.params);
      this.postMessage({
        type: "response",
        id: `resp_${Date.now()}`,
  ...
  private async executeRPC(method: string, params?: any): Promise<any> {
    switch (method) {
      ...
      case "getAgents": return this.getAgents(params);
      case "getTasks": return this.getTasks();
      case "submitTask": return this.submitTask(params);
      case "cancelTask": return this.bridge.callTool("submit_task", { description: `Cancel task ${params?.taskId}`, target: "cortex-0" });
      case "retryTask": return this.bridge.callTool("submit_task", { description: `Retry task ${params?.taskId}`, target: "cortex-0" });
      case "sendChatMessage": return this.sendChatMessage(params);
      case "getPendingApprovals": return this.getPendingApprovals();
      case "approveChange": return this.approveChange(params);
      case "rejectChange": return this.rejectChange(params);
      case "batchApprove": return this.batchApprove(params);
      case "searchMemory": return this.searchMemory(params);
  ...
  private async getAgents(_params?: any) {
  ...
  private async rejectChange(params: any) {
    try {
      await this.bridge.callTool("submit_task", {
        description: `Reject change: ${params.approvalId} — Reason: ${params.reason || "Rejected via sidebar"}`,
        target: "cortex-0",
      });
      return { success: true };
    } catch (err: any) {
